...
{
    PID_Calculate(&IMU_Temp_PID, BMI088.Temperature, RefTemp);
    IMUPWMSet(float_constrain(float_rounding(IMU_Temp_PID.Output), 0, (float)UINT32_MAX));
}
...
INS_t* INS_Init(void)
{
    ...
    // imu heat init
    PID_Param(&IMU_Temp_PID, 1000, 20, 0, Integral_Limit | Derivative_On_Measurement | OutputFilter, 0.9f, 0.0f, 300,
              2000);
...
void INS_Task(void)
{
    ...
    if ((count % 1) == 0)
    {
        ...
        VisionSetAltitude(INS.Yaw, INS.Roll, INS.Pitch);//@todo 根据C板放置位置，Pitch和Roll调换位置
    }
