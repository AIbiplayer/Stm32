...
typedef enum
{
    ...
    ID_punishment = 0x0104, // 裁判处罚数据
    ID_darts_data = 0x0105, // 飞镖数据
    ...
    ID_shoot_enable_num = 0x0208, // 可发射子弹数数据
    ID_rfid_state = 0x0209, // RFID状态数据
    ID_student_interactive = 0x0301, // 机器人间交互数据
    ID_key_data = 0x0304, // 机器人按键数据
    ID_maps_receive = 0x0308 // 地图数据接收
} CmdID_e;
...
/* ID: 0x0003  Byte:  16    己方机器人血量数据 */
typedef struct
{
    uint16_t Hero_1_HP; //英雄机器人1血量
    uint16_t Engineer_2_HP; //工程机器人2血量
    uint16_t Infantry_3_HP; // 步兵机器人3血量
    uint16_t Infantry_4_HP; // 步兵机器人4血量
    uint16_t Guard_7_HP; // 哨兵机器人7血量
    uint16_t Outpost_HP; // 前哨站血量
    uint16_t Base_HP; // 基地血量
} ext_game_our_robot_HP_t;
...
typedef struct
{
    ...
    uint8_t mains_power_gimbal_output : 1; //0为无输出，1为24v输出
    uint8_t mains_power_chassis_output : 1;
    uint8_t mains_power_shooter_output : 1;
} ext_game_robot_state_t;

/* ID: 0X0202  Byte: 12    实时功率热量数据 */
typedef struct
{
...
typedef struct
{
    uint8_t armor_id : 4; //装甲板受击ID
    uint8_t hurt_type : 4; //扣血类型：被攻击、裁判系统离线、初速度超限、热量超限、超功率、撞击
} ext_robot_hurt_t;
