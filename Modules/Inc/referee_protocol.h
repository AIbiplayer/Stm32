...
typedef enum
{
	...
	ID_reference_warn_data = 0x0104,	   // 裁判警告数据
	...
	ID_projectile_allowance = 0x0208,	   // 允许发射弹丸量数据
	ID_student_interactive = 0x0301,	   // 机器人间交互数据
	ID_small_map_interactive = 0x0303,	   // 小地图交互数据
} CmdID_e;
...
typedef enum
{
	...
	LEN_reference_warn_data = 3,				 // 0x0104
	...
	LEN_small_map_interactive = 15,							 // 0x0303
...
typedef struct
{
	uint8_t armor_id : 4; //当扣血原因为装甲模块被弹丸攻击、受撞击或离线时，该4 bit组成的数值为装甲模块或测速模块的ID编号；当其他原因导致扣血时，该数值为0
 	uint8_t HP_deduction_reason : 4; //0：装甲模块被弹丸攻击导致扣血,1：装甲模块或超级电容管理模块离线导致扣血,5：装甲模块受到撞击导致扣血
} ext_hurt_data_t;
...
typedef struct
{
	uint8_t bullet_type; //bit 1：17mm弹丸,bit 2：42mm弹丸
	uint8_t shooter_number; //发射机构ID： 1：17mm发射机构，2：保留位 ，3：42mm发射机构
	uint8_t launching_frequency; //弹丸射速（单位：Hz）
	float initial_speed; //弹丸初速度（单位：m/s）
} ext_shoot_data_t;

/* ID: 0x0208  Byte:  8    允许发弹量，固定以10Hz频率发送 */
typedef struct
{
...
/* ID: 0x0303  Byte:  12    选手端小地图交互数据，频率上限为3Hz  */
typedef struct
{
