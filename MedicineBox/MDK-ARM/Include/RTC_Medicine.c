...
static uint64_t FlashAlarm_Number_64 = 0; ///< 从Flash中找到闹钟的个数，64位，实际使用下面的8位
static uint8_t FlashAlarm_Number = 0;     ///< 从Flash中找到闹钟的个数
uint8_t Medicine_Remind_Status = 0;       ///< 服药提醒状态位，1为到点提醒
...
void Alarm_Operation(void)
{
    ...
    Flash_Read(&FlashAlarm_Number_64, RTC_AlarmNumber_ADDRESS); // 读取Flash中的闹钟个数
    FlashAlarm_Number = (uint8_t)FlashAlarm_Number_64;
