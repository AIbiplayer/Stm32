...
extern uint8_t Temp, Humid;            ///< 温湿度状态
extern uint8_t Light_Status;           ///< 夜间关灯状态
extern uint32_t ADC_Light_Data;        ///< 光敏值，范围为0~4095
extern uint8_t Day_Night_Status;       ///< 白天和夜晚状态，夜晚时为1
extern uint8_t Medicine_Remind_Status; ///< 服药提醒状态位，1为到点提醒
...
int main(void)
{
    ...
    HAL_TIM_Base_Start_IT(&htim5);                                                                            // 开启温度测量定时器
    HAL_TIM_IC_Start_IT(&htim3, TIM_CHANNEL_1);                                                               // 开启超声波测量
    __HAL_UART_ENABLE_IT(&huart2, UART_IT_IDLE);                                                              // 开启ESP01
    HAL_TIM_PWM_Start_DMA(&htim4, TIM_CHANNEL_1, (uint32_t *)LED_Array, sizeof(LED_Array) / sizeof(uint8_t)); // 开启灯光条PWM输出

    HAL_ADCEx_Calibration_Start(&hadc4, ADC_CALIB_OFFSET, ADC_SINGLE_ENDED); // 开启光敏传感器校准
    HAL_Delay(1);
    HAL_ADC_Start_DMA(&hadc4, &ADC_Light_Data, 1); // 开启光敏传感器DMA采集
    __HAL_ADC_ENABLE_IT(&hadc4, ADC_IT_AWD1);      // 开启光敏传感器ADC中断
    ...
    float Measure = 0;
    uint8_t Night_Breath_Status = 0; ///< 判断白天是否关闭夜灯，1为开启夜灯，使关灯只执行一次
    uint8_t Night_Light_Init = 0;    ///< 夜晚时开始始亮灯，后面不变，亮灯为1
    ...
    while (1)
    {
        ...
        if (Light_Status)
        {
            ...
            else if (ADC_Light_Data > 3600) // 夜间状态位
            {
            ...
            else if (Medicine_Remind_Status == 1) // 服药提醒判断
            {
                GRB_Breath(&Orange_Color);
                Voice_Remind(Voice_Reminder);
            }
            else if (Fan_Status == 1) // 制冷判断
            {
                for (uint8_t i = 0; i < 60; i++)
                {
            ...
            else if (ADC_Light_Data < 3550) // 白天关夜灯
            {
                Night_Light_Init = 0;
                GRB_Close();
